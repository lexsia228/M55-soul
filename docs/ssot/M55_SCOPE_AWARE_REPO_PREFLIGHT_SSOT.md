...
- new ChatGPT/Cursor/Codex/Grok session or cold start;
...
3. fresh `origin/main` / remote-main identity;
4. `docs/ssot/README.md` or equivalent SSOT index;
5. only the **relevant** normative SSOTs for the task;
6. relevant open PR / stacked branch / unmerged authority when it can contain newer Human decisions;
7. CLOSED GREEN / no-replay evidence relevant to the proposed work;
8. local branch/worktree/dirty facts when local mutation is proposed, otherwise declare `LOCAL_RUNTIME_UNAVAILABLE` if not observable;
9. duplication/supersession check before creating a new contract.
...
This is the default for an agent that is already executing an approved bounded lane such as UIUX.

Allowed only when all are true:

- same conversation/session or valid lane handoff;
- same assigned lane;
- same mutation owner;
...
- escalates only if it touches Creator Console, shared Creator surfaces, payment UX, global product truth, or another cross-lane authority.
...
execution_gate = <current executable gate>
relevant_ssot = <bounded set>
unmerged_relevant_authority = <PR/branch set or none>
...
For continuation work, a compact form is enough:
